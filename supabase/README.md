...
1. 按文件名顺序在 Supabase SQL Editor 运行 `migrations/` 中的迁移：
   ...
   - `202607280001_schedule_user_audio_cleanup.sql`（运行前先在 Vault 中创建名为 `cleanup_user_audio_secret_key` 的项目 Secret Key）
...
5. `user-audio` Bucket 必须保持为私有。生产 API 会使用服务端凭据写入以用户 ID 为前缀的临时对象，生成 5 分钟签名 URL 交给 AI 解析，并在每次解析尝试结束时删除对象。`cleanup-user-audio` Edge Function 使用 Storage API 删除超过 24 小时的孤立对象；Supabase Cron 在每小时第 17 分钟调用一次，并在 `apikey` 请求头中使用 Vault 保存的项目 Secret Key。
